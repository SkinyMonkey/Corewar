...
lProduct_ :: [[a]] -> [a] -> [[a]]
lProduct_ result pool = [ y ++ [x] | y <- result, x <- pool]

lProduct :: [[a]] -> [[a]]
lProduct pools = foldl lProduct_ [[]] pools

oneParameterFrom :: Parameter -> Parameter
oneParameterFrom parameter | parameter == register = Register 1
                        | parameter == indirect = Indirect 1
                        | parameter == direct = Direct 1

oneWrongParameterFrom :: Parameter -> Parameter
oneWrongParameterFrom parameter | parameter == register = Direct 1
                        | parameter == indirect = Register 1
                        | parameter == direct = Indirect 1

-- FIXME : [[register, indirect], [direct]] ->
--         [[register, direct], [indirect, direct]]
-- possible parameters -> combination of possible parameters

generateParameters :: (Parameter -> Parameter) -> [[Parameter]] -> [[Parameter]]
generateParameters f legalParameters = 
  let allCombinations = lProduct legalParameters
  in [ [ f legalParameter | legalParameter <- legalParametersCombination ] | legalParametersCombination <- allCombinations]

generateValidParameters :: [[Parameter]] -> [[Parameter]]
generateValidParameters legalParameters = 
  generateParameters oneParameterFrom legalParameters

generateInvalidParameters legalParameters = 
  generateParameters oneWrongParameterFrom legalParameters
...
validParameterTestCase instruction params = 
         ...
         in TestCase $ assertBool caseName $ checkParamsAgainstLegalParamSet instruction 4 params

invalidParameterTestCase instruction params = 
         let instructionName = opsNames !! (fromIntegral instruction - 1)
             parameters = intercalate " " $ map parameterName params
             caseName = "invalid case : " ++ instructionName ++ " " ++ parameters
         in TestCase $ assertBool caseName $ not $ checkParamsAgainstLegalParamSet instruction 4 params
...
--      parametersAreValid lld 4 [Direct 1] `shouldBe` False
    ...
    it "parametersAreInvalid" $ do
      ...
      runTestTT $ TestList $ testInvalidCases
...
--      parametersAreValid ld 4 [Register 1] `shouldBe` True
    ...
    it "firstParameterSetIsInvalid" $ do
      let instruction = instructionByMnemonic "sti"
      let tests = validParameterTestCase instruction [Register 1, Indirect 1, Direct 1]
      runTestTT $ TestList $ [tests]
      return ()
